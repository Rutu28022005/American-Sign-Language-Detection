...
class Application:
    ...
    def video_loop(self):
        try:
            ...
            # Debug: Print hand detection info
            if current_hand_detected:
                print(f"Hands detected: {len(hands)}")
                if hands and len(hands) > 0:
                    hand_info = hands[0]
                    print(f"Hand keys: {list(hand_info.keys()) if isinstance(hand_info, dict) else 'Not a dict'}")
            ...
            if hands:
                try:
                    # Fix: cvzone returns hands as a list, each hand is a dict
                    ...
                    if isinstance(hand, dict) and 'bbox' in hand:
                        ...
                        # Use the created white image instead of reading from file
                        white = self.white_img.copy()

                        handz, _ = hd2.findHands(image, draw=False, flipType=True)
                        self.ccc += 1
                        if handz and len(handz) > 0:
                            hand2 = handz[0]  # Get first hand from cropped image
                            if isinstance(hand2, dict) and 'lmList' in hand2:
                                ...
                                if self.current_symbol == "SPACE":
                                    ...
                                    self.status_label.config(text=f"Status: Added SPACE", fg="blue")
                                    ...
                                    self.status_label.config(text=f"Status: BACKSPACE", fg="red")
                                    ...
                                    self.status_label.config(text=f"Status: Added ENTER", fg="purple")
                                else:
    ...
    def predict(self, test_image):
        ...
        # Detect backspace gesture (closed fist)
        if (self.pts[6][1] > self.pts[8][1] and self.pts[10][1] > self.pts[12][1] and 
            self.pts[14][1] > self.pts[16][1] and self.pts[18][1] > self.pts[20][1] and
            self.pts[4][1] > self.pts[8][1] and self.pts[4][1] > self.pts[12][1] and
            self.pts[4][1] > self.pts[16][1] and self.pts[4][1] > self.pts[20][1]):
